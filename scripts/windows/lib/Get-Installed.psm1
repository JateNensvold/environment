function Select-List ($list, $listFilter) {

    $output = @()
    foreach ($filter in $listFilter) {
        $listMatches = @()
        $filterName = $filter.choco_name

        $listMatches += $list | Where-Object -Property "DisplayName" -Match -Value $filterName
        $matchCount = $listMatches.Length
...
function Get-InstalledPrograms () {
    $InstalledSoftware = choco list --local-only

    $result = $InstalledSoftware -split "`r`n"

    $output = @()
    ForEach ($line in $result[1..$result.Count]) {
        $lineArray = $line -split " "

        if ($lineArray.Count -eq 2) {
            $displayName = $lineArray[0]
            $version = $lineArray[1]
    ...
    # Return array of PSCustomObject (DisplayName, Version)
    return $output
}



# Found at the followign location https://www.powershellgallery.com/packages/ITPS.OMCS.Tools/1.8.4/Content/Scripts%5CGet-InstalledSoftware.ps1
